...
class FuzzerInterceptTest(unittest.TestCase):
    ...
    def test_payload_randomization(self):
        stdouts = []
        for i in range(15):
            commands = fuzzer_container.fuzz_file_or_folder("RANDOM", "/fuzzer/test/echo.php")
    ...
    def test_php_input_stream(self):
        commands = fuzzer_container.fuzz_file_or_folder("0", "/fuzzer/test/system_php_input.php")
        self.assertEqual(
    ...
    def test_sql_query(self):
        commands = fuzzer_container.fuzz_file_or_folder("1", "/fuzzer/test/mysql_query.php")
    ...
    def test_file_listing(self):
        ...
        self.assertTrue("/var/www/html/wp-content/test_file_GARLIC" in commands[0]["stdout"])

    def test_GET(self):
        commands = fuzzer_container.fuzz_file_or_folder("0", "/fuzzer/test/system_get.php")
        self.assertEqual(self._clean(commands[0]["stderr"]), "sh: 1: legitimateGARLIC: not found")

    def test_POST(self):
        commands = fuzzer_container.fuzz_file_or_folder("0", "/fuzzer/test/system_post.php")
        self.assertEqual(self._clean(commands[0]["stderr"]), "sh: 1: legitimateGARLIC: not found")

    def test_COOKIE(self):
        commands = fuzzer_container.fuzz_file_or_folder("0", "/fuzzer/test/system_cookie.php")
        self.assertEqual(self._clean(commands[0]["stderr"]), "sh: 1: legitimateGARLIC: not found")
    ...
    def test_include(self):
        ...
        self.assertTrue("GARLIC): failed to open stream: No such file or directory" in commands[0]["stdout"])

    def test_json_decode(self):
        commands = fuzzer_container.fuzz_file_or_folder("0", "/fuzzer/test/system_json_decode.php")
        self.assertEqual(self._clean(commands[0]["stderr"]), "sh: 1: legitimateGARLIC: not found")

    def test_file_get_contents(self):
        commands = fuzzer_container.fuzz_file_or_folder("0", "/fuzzer/test/file_get_contents.php")
        self.assertTrue("GARLIC): failed to open stream: No such file or directory" in commands[0]["stdout"])
    ...
    def test_patched_equality(self):
        commands = fuzzer_container.fuzz_file_or_folder("RANDOM", "/fuzzer/test/equality.php")
        counter = collections.Counter([self._clean(command["stdout"]) for command in commands])
    ...
    def test_patched_strict_equality(self):
        commands = fuzzer_container.fuzz_file_or_folder("RANDOM", "/fuzzer/test/strict_equality.php")
        counter = collections.Counter([self._clean(command["stdout"]) for command in commands])
